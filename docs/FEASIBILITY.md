...
- MVPの想定規模は OQ-012（同時1〜5人、1ユーザー100語、履歴50件）。アプリ内操作のp95目安は約2秒。翻訳・AI判定は timeout 8秒。CIのSLOゲートは作らない。
- T11: Workers Vitest の local D1 で所有80語を用意し、苦手集計query+1件抽選を計測した。当該テストは seed 込み 496ms。query+抽選はその内側。5秒超はhangとして落とす。約2秒のp95目安の内側であり、CIのSLOゲートではない。
...
困難判定はないため要件全体の差し戻しは不要。残未決は OQ-011（Chrome拡張）。OQ-012は2026-08-25決定。POC-05 の `issue-synonym` 外れは 2026-08-25 にMVP許容としたため、model/promptの差し戻しはしない。
...
- 2026-08-25 OQ-012の規模を記録（同時1〜5、1ユーザー100語、履歴50件）。p95応答目標は未決。この規模では FTS を必須にしない
- 2026-08-25 OQ-012のp95目安を約2秒として記録。対象はD1/アプリ内。翻訳・AI判定は timeout 8秒のまま対象外
