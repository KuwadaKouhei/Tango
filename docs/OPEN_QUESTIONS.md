...
| OQ-012 | 性能目標と想定規模 | p95応答、同時利用者、単語数、履歴数 | 本番公開判定前 | 一部決定（規模は決定。p95は未決） |
...
### OQ-012（2026-08-25 一部）

想定規模を決めた。p95 応答の数値目標はまだ決めない。利用者提示。
...
| p95 応答 | | **未決**。平均ではなく「遅い側の目安」。100回中95回が収まる秒数。個人MVPでは数値を持たなくてもよい |

この規模では一覧の FTS や集計テーブルは足さない。cursor 既定20/上限100はそのまま防御値。T11の local 80語計測（約500ms）は参考実測であり、p95 SLO にはしない。
...
- D1 Freeの容量上限は従来どおり。規模はOQ-012（同時1〜5人、1ユーザー100語、履歴50件）。p95 SLOは未決。
